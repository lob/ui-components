<template>
  <Modal
    :visible="visible"
    :close-button-aria-label="t('dropdown.confirmChangeModal.closeButtonAriaLabel')"
    @close="$emit('close')"
  >
    <div class="px-5 flex flex-col items-center">
      ...
      <div class="mb-2 type-xl-600 max-w-[246px] text-center">
        {{ title || t('dropdown.confirmChangeModal.text') }}
      </div>
      <div
      ...
      <div class="flex w-full justify-center">
        <LobButton
          ...
          @click="$emit('close')"
        >
          {{ t('dropdown.confirmChangeModal.goBack') }}
        </LobButton>
        <LobButton
          size="small"
          variant="secondary"
        ...
        >
          {{ confirmButtonText || t('dropdown.confirmChangeModal.confirmButtonText') }}
        </LobButton>
      </div>
    </div>
  </Modal>
